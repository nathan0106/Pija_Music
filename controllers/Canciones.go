...
// @Success 201 {int} models.Canciones
// @Failure 403 body is empty
// @router / [post]
func (c *CancionesController) Post() {
	var v models.Canciones
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddCanciones(&v); err == nil {
...
func (c *CancionesController) GetOne() {
	...
	v, err := models.GetCancionesById(id)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
...
func (c *CancionesController) GetAll() {
	...
	l, err := models.GetAllCanciones(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
...
func (c *CancionesController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Canciones{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateCancionesById(&v); err == nil {
